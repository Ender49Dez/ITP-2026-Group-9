...
#ifndef __SAM3X8E__
    // Use DIV64 to force an ultra-stable, slow SPI clock (250kHz on Uno, ~1.25MHz on ESP32)
    // This fixes "Could not detect PN532" and APDU packet corruption over flaky jumper wires.
...
void PN532_SPI::writeFrame(const uint8_t *header, uint8_t hlen, const uint8_t *body, uint8_t blen)
{
    ...
    if (length > 255) {
        write(0xFF); // Extended frame marker
        write(0xFF);
        uint8_t lenm = (length >> 8) & 0xFF;
        uint8_t leno = length & 0xFF;
        write(lenm);
        write(leno);
        write((uint8_t)(~(lenm + leno) + 1)); // LCS
    } else {
        write((uint8_t)length);
        write((uint8_t)(~length + 1));         // checksum of length
    }
