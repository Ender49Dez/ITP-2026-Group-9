...
bool EmulateTag::emulate(const uint16_t tgInitAsTargetTimeout){
  ...
  uint8_t command[] = {
        ...
        0x04, 0x00,         // SENS_RES (Generic Type 4A to prevent DESFire native commands)
        0x99, 0x11, 0x22,   // NFCID1 (NEW UID to bypass HONOR OS failure cache once more!)
        0x20,               // SEL_RES

        0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF, // NFCID2t (Invalid format disables FeliCa)
        0,0,0,0,0,0,0,0,                         // PAD
        0,0,                                     // System Code
  ...
  int16_t status = pn532.tgInitAsTarget(command, sizeof(command), tgInitAsTargetTimeout);
  if(status < 0){
    return false;
  }
  ...
  uint8_t compatibility_container[] = {
    0, 0x0F,
    0x20,
    0, 0xFF, // MLe (255 bytes - Universal standard)
    0, 0xFF, // MLc (255 bytes - Universal standard)
    0x04,       // T
    0x06,       // L
    0xE1, 0x04, // File identifier
    ((NDEF_MAX_LENGTH & 0xFF00) >> 8), (NDEF_MAX_LENGTH & 0xFF), // maximum NDEF file size
    0x00,       // read access 0x0 = granted
    0xFF        // write access 0xFF = deny
  };
  ...
  while(runLoop){
	...
	if(lc == sizeof(ndef_tag_application_name_v2) && 0 == memcmp(ndef_tag_application_name_v2, rwbuf + C_APDU_DATA, sizeof(ndef_tag_application_name_v2))){
	  setResponse(COMMAND_COMPLETE, rwbuf, &sendlen);
	} else{
	...
	break;
      case CC:
	if( p1p2_length >= sizeof(compatibility_container)){
	  setResponse(END_OF_FILE_BEFORE_REACHED_LE_BYTES, rwbuf, &sendlen);
	}else {
      uint16_t le = (lc == 0 || lc > 256) ? 256 : lc;
      bool is_eof = false;
      if (p1p2_length + le > sizeof(compatibility_container)) {
        le = sizeof(compatibility_container) - p1p2_length;
        is_eof = true;
      }
	  memcpy(rwbuf, compatibility_container + p1p2_length, le);
      
      if (is_eof) {
        setResponse(END_OF_FILE_BEFORE_REACHED_LE_BYTES, rwbuf + le, &sendlen, le);
      } else {
	    setResponse(COMMAND_COMPLETE, rwbuf + le, &sendlen, le);
      }
	}
	break;
      case NDEF: {
        uint16_t actual_length = (ndef_file[0] << 8) + ndef_file[1] + 2;
	if( p1p2_length >= actual_length){
	  setResponse(END_OF_FILE_BEFORE_REACHED_LE_BYTES, rwbuf, &sendlen);
	}else {
      uint16_t le = (lc == 0 || lc > 252) ? 252 : lc;
      
      bool is_eof = false;
      if (p1p2_length + le > actual_length) {
        le = actual_length - p1p2_length;
        is_eof = true;
      }
	  
      memcpy(rwbuf, ndef_file + p1p2_length, le);
      
      if (is_eof) {
        // HONOR OS strictly requires 62 82 when returning fewer bytes than requested!
        setResponse(END_OF_FILE_BEFORE_REACHED_LE_BYTES, rwbuf + le, &sendlen, le);
      } else {
        setResponse(COMMAND_COMPLETE, rwbuf + le, &sendlen, le);
      }
